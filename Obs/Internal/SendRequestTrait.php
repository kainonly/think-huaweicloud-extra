...
trait SendRequestTrait
{
    ...
    public function createV2SignedUrl(array $args = []): Model
    {
    ...
    private function createCommonSignedUrl(array $args = [], $signature): Model
    {
        ...
        $bucketName = isset($args['Bucket']) ? (string)$args['Bucket'] : null;
        $objectKey = isset($args['Key']) ? (string)$args['Key'] : null;
        $specialParam = isset($args['SpecialParam']) ? (string)$args['SpecialParam'] : null;
        $expires = isset($args['Expires']) && is_numeric($args['Expires']) ? (int)$args['Expires'] : 300;
    ...
    public function createV4SignedUrl(array $args = []): Model
    {
        if (!isset($args['Method'])) {
            $obsException = new ObsException('Method param must be specified, allowed values: GET | PUT | HEAD | POST | DELETE | OPTIONS');
            $obsException->setExceptionType('client');
    ...
    public function createV4PostSignature(array $args = []): Model
    {
        $bucketName = isset($args['Bucket']) ? (string)$args['Bucket'] : null;
        $objectKey = isset($args['Key']) ? (string)$args['Key'] : null;
        $expires = isset($args['Expires']) && is_numeric($args['Expires']) ? (int)$args['Expires'] : 300;
    ...
    private function checkMimeType($method, &$params): void
    {
    ...
    protected function makeRequest($model, &$operation, $params, $endpoint = null): Request
    {
        if ($endpoint === null) {
    ...
    protected function doRequest($model, &$operation, $params, $endpoint = null): void
    {
        $request = $this->makeRequest($model, $operation, $params, $endpoint);
        $this->sendRequest($model, $operation, $params, $request);
    }

    protected function sendRequest($model, &$operation, $params, $request, $requestCount = 1): void
    {
        $start = microtime(true);
        $saveAsStream = false;
    ...
    protected function doRequestAsync($model, &$operation, $params, $callback, $startAsync, $originMethod, $endpoint = null): \GuzzleHttp\Promise\PromiseInterface
    {
        $request = $this->makeRequest($model, $operation, $params, $endpoint);
        return $this->sendRequestAsync($model, $operation, $params, $callback, $startAsync, $originMethod, $request);
    }

    protected function sendRequestAsync($model, &$operation, $params, $callback, $startAsync, $originMethod, $request, $requestCount = 1): \GuzzleHttp\Promise\PromiseInterface
    {
        $start = microtime(true);
